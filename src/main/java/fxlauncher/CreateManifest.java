...
public class CreateManifest {
		...
		if (args.length > 3) {
			...
			if (named != null) {
				...
				if (named.containsKey("include-extensions"))
					includeExtensions.addAll(Arrays.stream(named.get("include-extensions").split(","))
			...
			for (String raw : params.getRaw()) {
				if (raw.startsWith("--cache-dir="))
					continue;
				if (raw.startsWith("--accept-downgrade="))
					continue;
				if (raw.startsWith("--include-extensions="))
					continue;
				if (raw.startsWith("--preload-native-libraries="))
					continue;
				if (raw.startsWith("--whats-new"))
					continue;
				if (raw.startsWith("--lingering-update-screen"))
					continue;
				if (rest.length() > 0)
					rest.append(" ");
				rest.append(raw);
	...
	public static FXManifest create(URI baseURI, String launchClass, Path appPath, PrivateKey key)
			throws IOException, URISyntaxException {
		...
		Files.walkFileTree(appPath, new SimpleFileVisitor<Path>() {
			public FileVisitResult visitFile(Path file, BasicFileAttributes attrs) throws IOException {
				if (!Files.isDirectory(file) && shouldIncludeInManifest(file)
					...
					manifest.files
							.add(new LibraryFile(appPath, file, uri.resolve(appPath.relativize(file).toString()), key));
	 ...
	 *
	 * Allthough the method is called setIncludeExtensions, it actually does an
	 * addAll.
	 *
